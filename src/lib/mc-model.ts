 ...
 * model's `display.head` transform + the head-item 0.625 scale on top. Robust by design — a face
 * with no resolvable texture renders a flat fallback colour, and a flat/`item/generated` model
 * renders as a textured sprite.
 *
 * Note: rendered as one mesh per face (not merged by material). A merged version was ~190× fewer
 * draw calls but changed depth/draw-order resolution of overlapping faces, which showed as a
 * one-sided artifact on models that aren't perfectly mirrored. Correctness wins here.
 */
